...
- **MvView** — материализованное представление; может состоять из нескольких частей (MvViewExpr), объединённых UNION ALL; опции уровня представления (например, `SKIP_DELETES`) хранятся в `options` и разбираются через **MvViewOption**.
- **MvViewOption** — описание допустимых параметров `OPTIONS` в `CREATE ASYNC MATERIALIZED VIEW` (сейчас: `SKIP_DELETES`, логический, по умолчанию `false`).
...
Реализован на ANTLR v4 (грамматика `YdbMatViewV1.g4`). Разбирает SQL-подобный скрипт в объекты модели (MvView, MvHandler). В определении MV поддерживаются необязательные предложения `DESTINATION` и `OPTIONS` (список пар «имя параметра — строковое значение»). Загрузка конфигурации — из файла или из таблицы YDB (настройки см. в README). Класс `MvConfigReader` вызывает парсер и возвращает `MvMetadata`.
...
- При включённой опции представления `SKIP_DELETES` (`MvView.isSkipDeletes()`) события `DELETE` из changefeed не попадают в очередь удаления: `deleteRows()` не вызывается, обработка `UPSERT` сохраняется. В режимах `SQL` / `SQL_DEBUG` сгенерированный `DELETE` помечается в `MvSqlPrinter` как пропускаемый во время выполнения.
