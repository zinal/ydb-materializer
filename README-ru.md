...
- Необязательное выражение `OPTIONS` задаёт параметры поведения представления (см. отдельный раздел ниже).
...
#### Параметры представления (OPTIONS)

Необязательное поведение представления задаётся предложением `OPTIONS`. Оно может следовать за `DESTINATION` и предшествовать `AS`:
...
|----------|-----|--------------|----------|
| `SKIP_DELETES` | логический | `false` | При значении `true` события `DELETE` из всех входных changefeed игнорируются, операции `DELETE` в таблице MV не выполняются. Обработка `UPSERT` не меняется. |

Логические значения нечувствительны к регистру. Допускаются формы `true` / `false`, `yes` / `no`, `1` / `0`, а также `y` / `n` / `t` / `f`.
...
При включённом `SKIP_DELETES`:

- Строки MV по-прежнему обновляются по событиям `UPSERT` из входов `STREAM` и `BATCH`, в том числе при изменениях в присоединённых таблицах.
- События `DELETE` в исходных таблицах не переносятся в MV; существующие строки MV сохраняются, пока их не удалят вручную или не перезапишут последующим `UPSERT` с тем же ключом MV.
- MV может расходиться с результатом определяющего `SELECT` по актуальным исходным данным после удаления строк в источниках или нарушения условий соединения.

Параметр уместен, когда удаления в исходных таблицах не должны убирать денормализованные строки MV (например, только добавление фактов или мягкое удаление, когда MV рассматривается как исторический снимок).

В режимах вывода `SQL` и `SQL_DEBUG` сгенерированные операторы `DELETE` помечаются как пропускаемые во время выполнения, если параметр включён.
...
- `job.input.table` — имя таблицы для инструкций (для режима TABLE). Поддерживается устаревшая схема (`statement_no`, `statement_text`) и модульная схема с колонкой `module_id` и первичным ключом `(module_id, statement_no)`: корневой модуль `module_id=''` (общие определения), модуль обработчика — `module_id` равен имени обработчика. При запуске обработчика читаются только корневой модуль и модуль этого обработчика; устаревшая схема всегда загружает все строки. Формат определяется автоматически по описанию таблицы.
...
- `job.apply.queue.percent` — процент очереди apply, резервируемый для интерактивных (непакетных) операций (по умолчанию: 40)
...
- **`job.apply.queue.percent` / `applyQueuePercent`**
  - Процент размера очереди apply, резервируемый для интерактивных (STREAM/CDC) операций. Значение по умолчанию: `40`. Пакетные операции (сканирование, обновление по изменениям справочников) в обычном (нефорсированном) режиме не могут занять больше `applyQueueSize * (100 - applyQueuePercent) / 100` элементов очереди. Интерактивные операции по-прежнему могут использовать всю очередь вплоть до `applyQueueSize`. Это не меняет порядок обработки внутри воркера (`drainTo` забирает текущий backlog целиком), но не даёт пакетному входу удерживать очередь полностью занятой и блокировать постановку STREAM-задач. Форсированная постановка порождённых задач (для избежания взаимоблокировок при разбиении по ключам) по-прежнему может временно превысить лимиты, но учитывается в счётчиках очереди.
